...
class Users_Model extends CI_Model{
	...
	function get_all_menu($idR=NULL,$perpage=NULL,$position=NULL,$wilayah=NULL,$harga=NULL,$menu=NULL){
		$this->db->distinct();	
	...
	function get_all_restoran(){
		$this->db->select('a.*,b.cStatusOpen');
		$this->db->from('msusers a');
	...
	function get_menu_detail($resto_name,$menu_name){
		/*
		...
		*/
		...
		$this->db->select('a.id,a.cNama,a.cImageLarge,a.cImageThumb,d.iHarga,c.cNama as cNamaResto,b.iMinOrder,b.iCharge,b.iDeliveryTime, c.id as iRestoId,b.id as idRestoProfile');
		$this->db->from('msmenurestos a');
		$this->db->join('msrestprofiles b', 'a.idRestoran= b.id');
		$this->db->join('msusers c', 'b.iUsers= c.id');		
		$this->db->join('mshargamenus d', 'a.id= d.idMenu');
	...
	function get_menu_restoran($id){//-- untuk dipakai di halaman build_order untuk menampilkan menu di restoran tertentu 
		...
		$query=$this->db->query("select a.id, a.cNama, coalesce(b.iHarga,0) iHarga, a.cImageThumb, a.cImageLarge, a.cDesc
								from msmenurestos a
								left join 
								(	
									select *
									...
									on a.id= b.idMenu 
								where a.cStatus='Y' and a.idRestoran='".$id."' order by a.id
							");
		return $query;
	...
	function get_user_profile($id,$type,$session=NULL){
		if($type=='customer'){
			$this->db->select('b.cNama,b.cEmail,a.cAlamat,a.cHp,a.cTelp,b.cEmailNew,a.cWilayah,c.cWilayah as cNamaWilayah');
			...
			$this->db->select('b.cNama,b.cEmail,a.cHp,a.cTelp,a.iMinOrder,a.iCharge,a.cStatusOpen,a.cStatusAntar,a.cStatusTax,a.cDescription,a.cImage,b.cEmailNew');
			$this->db->from('msrestprofiles a');
			$this->db->join('msusers b', 'a.iusers = b.id');	
			if($session!=NULL){
				$this->db->where('b.cActivateCode', $session); 
	...
	*/
	...
	function get_charge(){
		...
		$this->db->from('mssystables');
		$this->db->where('cMasterCode', 'Delivery');
		$this->db->where('cChildCode', 'Charge');
		$this->db->where('cStatus', 'Y');		
        return $this->db->get();
    }
	...
	function add_order($id,$jumlah,$desc=NULL){
        $data_common=array(
	...
	function get_order_session(){
		$this->db->select('a.cSession,a.iMenuId,sum(a.iJumlah)iJumlah,b.cNama,c.iHarga');
	...
	function get_order_session_check_out(){
		$this->db->select('a.cSession,a.iMenuId,sum(a.iJumlah)iJumlah,b.cNama,c.iHarga,a.cDesc');
		$this->db->from('trorderdetails_tmp a');
		$this->db->join('msmenurestos b', 'a.iMenuId= b.id');
		$this->db->join('mshargamenus c', 'b.id= c.idMenu');				
		$this->db->where('a.cSession',$this->session->userdata('cOrderSess')); 
    ...
    function add_trorders_tmp($iTax){
		$this->db->where('cSession', $this->session->userdata('cOrderSess'));
        ...
        if(!mysql_errno()) {
			...
			$data_common=array(
				...
				'iTax'=>$iTax,
				'cAntarBy'=>$this->input->post('deliver_method',TRUE),			
				'iBiayaAntar'=>$this->session->userdata('iCharge'),
				'cJenisTerimaMakanan'=>$this->input->post('del_option',TRUE),			
	...
	*/
    ...
    function add_email(){
        $data_common=array(
            'cEmail'=>$this->input->post('signup-email',TRUE),
            'iPoin'=>'50',
			'cStatus'=>'Y',
			'cCreated'=>'system',						
            'dCreated'=>date('Y-m-d H:i:s')
	...
	function customer_point(){		
		$query=$this->db->query("
			select a.idCust,coalesce(a.iPoint,0) iPoint from
			(
	...
	function customer_redeem_point(){		
		$query=$this->db->query("
	...
	function sign_up_point(){		
		$query=$this->db->query("select iPoin
				from msemail
				where cEmail='".$this->session->userdata('cEmail')."' and cStatus='Y'"); 
    ...
    function add_ms_voucher($value,$code,$jenis,$idR=NULL){
        $data_common=array(
			...
			'idRestoran'=>$idR,
            'dStartBerlaku'=>date('Y-m-d'),
            'dEndBerlaku'=>date('Y-m-d',time()+30*24*3600),
            'cStatusUsed'=>'N',
            'cStatus'=>'Y',
			'cCreated'=>'system',
            'dCreated'=>date('Y-m-d H:i:s')
	...
	function report_detail($awal, $akhir){
		$query=$this->db->query("select a.idOrders,a.iMenuId, a.iJumlah, c.iHarga
					from trorderdetails a
					left join msmenurestos b
						on a.iMenuid=b.id
					left join mshargamenus c
