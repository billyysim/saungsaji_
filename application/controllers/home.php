...
class Home extends CI_Controller{
    ...
    function index(){
		...
		$open_ss=$this->users_model->get_sys_table('Open_SS',NULL)->row();
		$close_ss=$this->users_model->get_sys_table('Close_SS',NULL)->row();
		$sess_random=array('crandom'=>rand(1,6),'open'=>$open_ss->cValue,'close'=>$close_ss->cValue);
    	$this->session->set_userdata($sess_random);
    	$sess_data=array('cSearch'=>'');
    ...
    function build_order($resto=NULL){
		...
		if($this->session->userdata('idRestoran')!=""){
    ...
    function checkout_one_stop(){
		...
		$jum_order = $this->db->query("select * from trorderdetails_tmp where cSession='".$this->session->userdata('cOrderSess')."'");		
		if($jum_order->num_rows()>0)
			$this->redirects('checkout_one_stop',$title);
		else
            redirect($this->base."/index.php");
    }
	...
	*/
    ...
    function contact(){
        ...
        $this->form_validation->set_rules('email', 'Wajib diisi', 'trim|required|valid_email');
        $this->form_validation->set_rules('hp', 'Wajib diisi', 'trim');
        $this->form_validation->set_rules('desc', 'Wajib diisi', 'trim|required');

        if($this->input->post('submit')){
            if($this->form_validation->run() == FALSE){
				$data['msg']="<font color='red'>Cek kembali data yang anda inputkan.</font>";                                    
            }
            else{
                if($this->users_model->add_contact_us()){					
					$subject='Info at SaungSaji - Do not reply';
                    $message = "<strong>Dear ".$this->input->post('name',TRUE).",</strong><br><br>
							<p>Terima kasih telah menghubungi <a href='www.saungsaji.com'>SaungSaji.com</a> <br><br>
							Staff kami akan segera menghubungi anda.</p>
							<br><br>
					...
					if($this->send_email($subject,$message,$from,$to)){
						$this->send_email($subject,$this->input->post('desc',TRUE),$from,'info@saungsaji.com');
						$data['msg']="Terima kasih telah menghubungi. <br>Staff kami akan segera menghubungi anda";                    
					}					
                }
                else{
                    $data['msg']="<font color='red'>Error dalam pengiriman data. Silakan untuk mengulang</font>";                    
                }
            }
        }
    ...
    function faq(){
		$this->load->model('users_model', '', TRUE);
        $data['base']=$this->base;
		$data['min_order']=$this->users_model->get_sys_table('Minimal',NULL)->row();
		$data['charge_delivery']=$this->users_model->get_charge()->row();
		$data['open_ss']=$this->users_model->get_sys_table('Open_SS',NULL)->row();
		$data['close_ss']=$this->users_model->get_sys_table('Close_SS',NULL)->row();
	...
	function change_password(){
        ...
        $this->form_validation->set_rules('old_pass', ' Password lama', 'trim|required|callback_password_check');
        ...
        if($this->input->post('reset')){
            if($this->form_validation->run() == FALSE){
		        ...
		        //$this->load->view('resetpassword',$data);
					...
					$subject='Password Reset at SaungSaji - Do not reply';
	...
	*/
	...
	function redirects($view=NULL,$title=NULL,$nama=NULL){
		...
		if($view=='home'){
            $data['menu'] = $this->users_model->get_all_menu(0,NULL,NULL,NULL,NULL,NULL,1,'N','N');
			$data['menu_new'] = $this->users_model->get_all_menu(0,NULL,NULL,NULL,NULL,NULL,1,'Y',NULL);
			$data['menu_deal'] = $this->users_model->get_all_menu(0,NULL,NULL,NULL,NULL,NULL,1,'N','Y'); //yg di dealkan adalah menu lama
			...
			$data['alamat'] = $this->users_model->get_alamat($this->session->userdata('idRestoran'));
			$data['pengantaran'] = $this->users_model->get_pengantaran($this->session->userdata('idRestoran'));
			...
			$data['min_order']=$this->users_model->get_sys_table('Minimal',NULL)->row();				
			$data['del_time']=$this->users_model->get_sys_table('Delivery','Time')->row();				
			$data['del_max']=$this->users_model->get_sys_table('Delivery','Max')->row();				
			if(!$this->input->post('login_submit')){
				$data['cap']=$this->get_captcha();
			}
		}
			...
			$data['wilayah'] = $this->users_model->get_all_wilayah();
			if ($this->session->userdata('userType')<>""){
				$data['user_profile']=$this->users_model->get_user_profile($this->session->userdata('userID'),$this->session->userdata('userType'))->row();
				//$data['menu_diskon'] = $this->users_model->get_menu_diskon(NULL,NULL,NULL);
				...
				$data['order_temp'] = $this->db->query("select a.id, a.iTax,a.iBiayaAntar,a.dTglOrder,
					a.cJenisTerimaMakanan,a.dJamTerima,a.cVoucherCode,a.cAlamatAntar,a.cDesc,a.iJenisLokasiAntar,b.cPayment,c.iNominal,d.cNama
					from trorders_tmp a
					left join mspaymentmethods b on a.iPaymentId=b.id
					left join msvouchers c on a.cVoucherCode=c.cCode
					left join msusers d on a.idRestoran=d.id
		...
		else if($view=='checkout_one_stop'){			
			$data['user_profile']=$this->users_model->get_user_one_stop($this->session->userdata('cOrderSess'))->row();
			$data['order_check_out']=$this->users_model->get_order_session_check_out();
			$data['order_temp'] = $this->db->query("select a.id, a.iTax,a.iBiayaAntar,a.dTglOrder,
				a.cJenisTerimaMakanan,a.dJamTerima,a.cVoucherCode,a.cAlamatAntar,a.cDesc,a.iJenisLokasiAntar,b.cPayment,c.iNominal,d.cNama
				from trorders_tmp a 
				left join mspaymentmethods b on a.iPaymentId=b.id
			...
			if($this->session->userdata('userID')==""){
				$data['order_finish'] = $dataorder = $this->db->query("select a.id, a.idOneStop,a.idCust,a.iTax,a.iBiayaAntar,a.cJenisTerimaMakanan,
					...
					b.cPayment,c.cNama,c.cHp,c.cTelp,c.cAlamat as cAlamatOneStop,e.cAlamat as cAlamatCustomer,f.iNominal, c.cEmail, g.cNama as cNamaRestoran,
					g.cEmail as cEmailRestoran
					from trorders a 
					left join mspaymentmethods b on a.iPaymentId=b.id					
				...
				$data['order_finish'] = $dataorder = $this->db->query("select a.id, a.idOneStop,a.idCust,a.iTax,a.iBiayaAntar,a.cJenisTerimaMakanan,
					a.dJamTerima,a.cVoucherCode,a.cAlamatAntar,a.cDesc,a.iJenisLokasiAntar,a.dTglOrder,
					b.cPayment,c.cAlamat as cAlamatOneStop,d.cNama,e.cAlamat as cAlamatCustomer,e.cHp,e.cTelp,f.iNominal, d.cEmail, g.cNama as cNamaRestoran,
					g.cEmail as cEmailRestoran, h.cWilayah as cNamaWilayah
					from trorders a
					left join mspaymentmethods b on a.iPaymentId=b.id
					left join msonestop_cust c on a.idOneStop=c.id
					left join msusers d on a.idCust=d.id
					...
					left join msvouchers f on a.cVoucherCode=f.cCode
					left join msusers g on a.idRestoran=g.id
					left join mswilayahs h on e.cWilayah=h.id
			...
			//substr('00000'.$dataorder->id,-5)
			$subject='Order at SaungSaji - Do not reply';
			$message = "<div style='font-family: verdana; font-size:12px;'>
							...
							<div>
								...
								<div style='margin-top:20px;'>Terima kasih telah memesan makanan melalui SaungSaji.com
									<div style='margin-top:5px;'>Staff kami akan segera menghubungi anda</div>
								</div>
							</div>
							<div style='margin-top:20px;'>
								<table cellpadding=3px cellspacing=0 style='font-size:12px;'>
									...
									<tr><td>Order Invoice</td><td>#".$dataorder->id."-".date('Ymd',strtotime($dataorder->dTglOrder))."</td></tr> 
			...
			$message .="<tr bgcolor='#FFFFEO'>
				...
				if($dataorder->idOneStop!="NULL" && $dataorder->idOneStop!="0")
					...
					if ($dataorder->iJenisLokasiAntar==1)
						$alamat = $dataorder->cAlamatCustomer." - ".$dataorder->cNamaWilayah;
				...
				<div style='margin-top:20px;'>
					...
					<tr>
						<td>Waktu Terima</td>
						<td>".$jenis." - ".$Hari.", ".date('d-m-Y H:i',strtotime($dataorder->dJamTerima))."</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>		
			...
			if($this->users_model->get_email($dataorder->id,$dataorder->cEmail)->num_rows() < 1){			
				...
				//kirim email order ke restoran
				if($this->send_email($subject,$message,$from,$toRestoran)){					
					$this->users_model->add_send_email_tr($dataorder->id,$dataorder->cEmailRestoran);
				}
			}
				...
				if($this->form_validation->run() == TRUE){				
					if($this->input->post('payment_method',TRUE)!=0){
						if($this->input->post('del_option',TRUE)=='1'){
							...
							$waktu_jam = date('H:i',strtotime($time));	
							...
							$del_time=$this->users_model->get_sys_table('Delivery','Time')->row();				
							...
							//echo $waktu_jam . "<br>";
							//echo date('H:i',strtotime($get_waktu_restoran->dOpenHour)). "<br>";
							//echo date('H:i',strtotime($get_waktu_restoran->dCloseHour));
							$service=$this->input->post('delivery',TRUE);
						
							if($waktu>date('Y-m-d H:i') && $minutes>$del_time->cValue){ //lebih dari jam sekarang -- date('Y-m-d H:i',strtotime($time))>date('Y-m-d H:i')								
								$flagCheck=0;
								if($waktu_jam>date('H:i',strtotime($get_waktu_restoran->dOpenHour)) && $waktu_jam<date('H:i',strtotime($get_waktu_restoran->dCloseHour))){ // date('H:i',strtotime($time)) -- antara jam buka restoran
									$flagCheck=0;
								}
								else{
									$flagCheck=2;
						...
						if($flagCheck==0){
							...
							*/
							...
							$del_max=$this->users_model->get_sys_table('Delivery','Max')->row();	
							if($this->session->userdata('iCharge')*$subtotal/100>$del_max->cValue){
								$service=$del_max->cValue;
							}	
							else{
								$service=$this->session->userdata('iCharge')*$subtotal/100;
							}			
							//===========================================================================================================
							if($this->input->post('remain',TRUE)>0){
								$data['pesan']="<font color='red'>Minimal Order Rp. ".number_format($this->input->post('min_order',TRUE),0,",",".")."</font>";										
							}
							else{
								if($this->session->userdata('userID')!="" && $this->input->post('payment_method',TRUE)==2)
								{
									$wallet=$this->users_model->wallet()->row();
									if($wallet->wallet-$subtotal-$iTax-$service>=0)
									{
										if($this->users_model->add_trorders_tmp($iTax,$subtotal,$service))
							...
							$data['pesan']="<font color='red'>Waktu pemesanan minimal ".$del_time->cValue." menit ke depan</font>";								
						}						
						else if($flagCheck==2){
					...
					//===========================================================================================================						
					//Ditutup karena sudah berubah business process, tidak ada fitur ambil sendiri dan tidak ada fitur pilih siapa yang antar makanan
					/*
					if($this->input->post('del_option',TRUE)=='1'){
						if ($this->input->post('deliver_method',TRUE)!='0'){
							if ($this->input->post('deliver_method',TRUE)=='SaungSaji'){										
								if($this->session->userdata('userID')!="" && $this->input->post('payment_method',TRUE)==2)
								{
									$wallet=$this->users_model->wallet()->row();
					...
					*/
					...
					$data['pesan']="<font color='red'>Isikan data yang dibutuhkan</font>";
			}
			else{
		...
		if($this->input->post('btncontinue')){
			...
			$this->form_validation->set_rules('nama', 'Wajib diisi', 'trim|required');
			...
			$this->form_validation->set_rules('region', ' ', 'trim|required|is_natural_no_zero');
			...
			if($this->form_validation->run() == FALSE){											
				...
				if($this->users_model->add_tmp_order_transaction_one_stop()){ 									   
					//$this->send_email_transaction();
					redirect($this->base."/index.php/home/checkout_one_stop");
				}					
				else
		...
		if($this->input->post('btnsubmit')){						
			if($this->input->post('address',TRUE)==2){
				$this->form_validation->set_rules('address2', 'Wajib diisi', 'required|trim');
				$this->form_validation->set_rules('region', ' ', 'required|trim|is_natural_no_zero');
				if($this->form_validation->run() == false){
					$data['pesan']="Cek kembali ALAMAT dan WILAYAH pengantaran";
				}
				else{
		...
		if($this->input->post('login_submit') || $this->input->post('login_submit_')){				
			...
			$this->form_validation->set_rules('password', 'Wajib diisi', 'required|trim');		
			
            if($this->form_validation->run() == false){
				//echo 'test';
				//exit;
				$data['msg']="Cek kembali email dan password yang anda masukkan";
				$this->load->view('header20150331', $data);
				//$this->load->view($view, $data);
				if($this->input->post('login_submit')){
                ...
                if($user != NULL){
					...
					elseif($type=='admin'){
						...
						redirect($this->base."/index.php/a/home");				
					}
        ...
        if($this->input->post('signup_submit')){
            if($this->form_validation->run() == FALSE){
                ...
                if($this->users_model->add_user(1,$new_pass)){					
					$subject='Registration at SaungSaji - Do not reply';
                    $message = "<strong>Dear ".$this->input->post('name',TRUE).",</strong><br><br>
    ...
    function cart($resto,$menu,$deal=NULL,$submit=NULL,$jumlah=NULL,$harga=NULL,$diskon=NULL,$desc=NULL){
        //$this->login_check();
        $this->load->model('users_model', '', TRUE);
        $data['base']=$this->base;
        $data['title']="SaungSaji.com - online food order and delivery";
		...
		if($this->users_model->get_menu_detail($resto,$menu)->num_rows() > 0){		
			...
			if($submit=='submit'){
				...
				$this->session->set_userdata($sess_resto);
				//$resto_id=$this->session->userdata('idRestoran');	
				$this->users_model->add_order($id_first_order['id'],$jumlah,urldecode($desc),$harga,$diskon);
				//$this->redirects('build_order',$title,$id_first_order['cNamaResto']);				
				redirect($this->base."/index.php/home/build_order/".$resto."/#menu");
			}
			else{
				if($deal=='deal'){
					$data['menu_detail'] = $menu_detail = $this->users_model->get_menu_detail($resto,$menu,'deal')->row_array();
				}
				else{
				...
				$data['min_order']=$this->users_model->get_sys_table('Minimal',NULL)->row();			
				$data['pengantaran'] = $this->users_model->get_pengantaran_byname($resto);								
    ...
    function get_captcha($type=null){
        ...
        $this->form_validation->set_rules('captcha', 'Wajib diisi', 'trim|required|callback_captcha_check');
        $vals = array(
    ...
    function captcha_check()
    {
        ...
        if ($row->count == 0)
        {
            $this->form_validation->set_message('captcha_check', 'Captcha salah!');
            return FALSE;
        }else
		{
            return TRUE;
        ...
        if($this->input->post('submit_profile')){
            if($this->form_validation->run() == FALSE){
                ...
                if($this->users_model->edit_user($this->session->userdata('userID'))){                    
					$data['user_profile']= $user = $this->users_model->get_user_profile($this->session->userdata('userID'),$this->session->userdata('userType'))->row();
					...
					if($this->session->userdata('cEmail')!=$this->input->post('email',TRUE)){					
						...
						if($this->send_email($subject,$message,$from,$to)){
							$data['msg1']="<div style='margin-bottom:5px;'>Data profile anda telah diupdate</div>
		...
		*/
		$data['order_details']= $this->db->query("select b.* 
			...
			(
				...
				on a.id=b.idOrders
			where a.cStatus='Y' and a.idCust='".$this->session->userdata('userID')."' and (a.iStatusAntar=".$id1." or a.iStatusAntar=".$id2.")");
		...
		$data['order']= $this->db->query("select ".$position." as posisi,id,cNama,dTglOrder,cPayment,iTax+iBiayaAntar-iNominal as iTotal,
				...
				order by a.dTglOrder desc, a.id asc limit ".$position.",".$item_per_page.""); 				
	...
	function mywallet(){ //belum dikurangi voucher
		...
		$data['msg']="";
		$data['user_menu']=$this->users_model->get_user_menu(1);
		$data['bank_company']=$this->users_model->get_sys_table('Bank',NULL);
	...
	function mypoint($warning=NULL){
		...
		elseif ($warning=='success')
			$data['msg']="Poin telah ditukarkan. Silakan cek My Point - Transaction untuk melihat kode voucher anda";
		else
			$data['msg']="";
		$data['user_menu']=$this->users_model->get_user_menu(1);
		$data['user_point']=$this->users_model->customer_point($this->session->userdata('userID'))->row();
		//sign up point sudah digabung ke user point
		//$data['sign_up_point']=$this->users_model->sign_up_point($this->session->userdata('cEmail'))->row();
		$data['user_redeem_point']=$this->users_model->customer_redeem_point($this->session->userdata('userID'))->row();
	...
	function redeem(){
		...
		$data['redeem']= $this->db->query("select ".$position." as posisi, a.dTglRedeem, b.cNama,b.iPoin, a.cCode, a.dStartBerlaku, a.dEndBerlaku
	...
	function add_order(){
		...
		$price_disc=explode("-", $this->input->post('pr',TRUE));// harga dan diskon				
		$idRest=$this->input->post('idRest',TRUE);
		$desc=$this->input->post('req',TRUE);// description pesanan
		//$type=$this->input->post('type',TRUE); // type delivery -- tidak digunakan lagi
		//$del_by=$this->input->post('del_by',TRUE);// diantar oleh
		$tax=0;
		$total=0;
		$charge=0;
		$sisa=0;
		...
		$this->load->model('users_model', '', TRUE);			
		$menu_diskon = $this->users_model->get_menu_diskon(NULL,NULL,NULL);
		$min_order=$this->users_model->get_sys_table('Minimal',NULL)->row();				
		$del_max=$this->users_model->get_sys_table('Delivery','Max')->row();
		
		if ($stat == 'add')
			$kondisi = $this->users_model->add_order($id,$jumlah,$desc,$price_disc[0],$price_disc[1]);
		else if ($stat == 'del'){
        ...
        if($kondisi){
			...
			if ($restoran->cStatusTax=='Y')
			{
				$tax=0.1*$subtotal;
				$output .="<div class='order-myitem-row'>";
				$output .="<div class='order-myitem-col1'>PPN</div>";
				$output .="<div class='order-myitem-col4'>Rp</div>";
			...
			$output .="<div class='order-myitem-row'>";
			$output .="<div class='order-myitem-col1'>Jasa Layanan</div>";
			$output .="<div class='order-myitem-col4'>Rp</div>";
			$output .="<div class='order-myitem-col2'>";
			...
			$output .="<input type='text' readonly class='currency myitem-price' name='delivery' id='delivery' value='".number_format($service,0,",",".")."'>"; 
			$output .="</div>";
			$output .="<div class='order-myitem-col3'>&nbsp;</div>";
			$output .="</div>";
			...
			*/
			...
			if ($subtotal<$min_order->cValue){
				$sisa=$min_order->cValue-$subtotal;
				$output .="<div class='order-myitem-row red'>";			
				$output .="<div class='order-myitem-col1 red'>Masih Kurang</div>";
				$output .="<div class='order-myitem-col4 red'>Rp</div>";
				$output .="<div class='order-myitem-col2'>";
				$output .="<input type='text' readonly class='currency myitem-price red' name='remain' id='remain' value=".number_format($sisa,0,",",".").">";
				$output .="<input type='hidden' name='min_order' id='min_order' value=".$min_order->cValue.">";
				$output .="</div><div class='order-myitem-col3'>-</div></div>";
			}
			/*
			if ($restoran->cStatusAntar=='Y' && $subtotal<$restoran->iMinOrder){
				$sisa=$restoran->iMinOrder-$subtotal;
			}
			if($del_by!="SaungSaji" && $type=="1" && $sisa>0){
				$output .="<div class='order-myitem-row red'>";			
				$output .="<div class='order-myitem-col1 red'>Masih Kurang</div>";
				$output .="<div class='order-myitem-col4 red'>Rp</div>";
				$output .="<div class='order-myitem-col2'>";
				$output .="<input type='text' readonly class='currency myitem-price red' name='remain' id='remain' value=".number_format($sisa,0,",",".")."></div>";
	...
	function check_search(){
		...
		$sess_search=array('cSearch'=>$search);
	...
	function by_page(){
		...
		$idM=$this->session->userdata('cSearch');//$this->input->post('idM',TRUE);				
		$page_number = filter_var($this->input->post('page',TRUE), FILTER_SANITIZE_NUMBER_INT, FILTER_FLAG_STRIP_HIGH);
		//throw HTTP error if page number is not valid
		if(!is_numeric($page_number)){
			header('HTTP/1.1 500 Invalid page number!');
		...
		$this->load->model('users_model', '', TRUE);
        $data['menu'] = $this->users_model->get_all_menu($idR,$item_per_page,$position,$idW,$idH,$idM,1,'N','N');		
		$data['menu_diskon'] = $this->users_model->get_menu_diskon(NULL,NULL,NULL);
	...
	function by_restaurant(){
		...
		$idW=$this->input->post('idW',TRUE);		
		$idH=$this->input->post('idH',TRUE);
		$idM=$this->session->userdata('cSearch');		
		//$idM=$this->input->post('idM',TRUE);				
		$item_per_page=6;
		$menu= $this->users_model->get_all_menu($idR,NULL,NULL,$idW,$idH,$idM,1,'N','N');		
	...
	{		
		...
		if($status==NULL){
			...
			$dataorder = $this->db->query("select a.id, a.idOneStop,a.idCust,a.iTax,a.iBiayaAntar,a.cJenisTerimaMakanan,a.dJamTerima,f.cCode cVoucherCode,
						...
						left join mscustprofiles e on d.id=e.iUsers
						left join msvouchers f on a.cVoucherCode=f.cCode 
							where a.id='".$this->input->post('id',TRUE)."'")->row(); // and f.idRestoran='".$this->session->userdata('userID')."' and f.cJenisVoucher='Resto'
		}
		...
		$output .= "<div class='checkbox-row'>";
		$output .= "<div class='myorder-detail-desc'>PPN</div>";
		$output .= "<div class='myorder-detail-price'>Rp. ".number_format($dataorder->iTax,0,",",".")."</div>";
		$output .= "</div>";
		$output .= "<div class='checkbox-row'>";
		$output .= "<div class='myorder-detail-desc'>Jasa Layanan</div>";
    ...
    function redeem_point($nama,$submit=NULL,$jumlah=NULL,$desc=NULL){
		...
		$user_point=$this->users_model->customer_point($this->session->userdata('userID'))->row();
		//point yang telah ditukarkan
		$user_redeem_point=$this->users_model->customer_redeem_point($this->session->userdata('userID'))->row();
		//default 1 poin
		$point_trans=$this->users_model->point_trans_sys()->row();		
		$reward_detail=$this->users_model->get_reward_detail($nama)->row_array();
		//point ketika daftar counting down-- sudah digabung ke customer point
		//$sign_up_point=$this->users_model->sign_up_point($this->session->userdata('cEmail'))->row();
		...
		$order_count = $this->users_model->customer_order(2,2);//browse data transaksi
		
		if($this->users_model->get_reward_detail($nama)->num_rows() > 0){
			if($submit=='submit'){
				//if($order_count->num_rows()>0){//sudah ada transaksi baru bisa tukar point
					if($poin_usr-($reward_detail['iPoin'])>=0){
						$reward_detail=$this->users_model->get_reward_detail($nama)->row_array();
						$code=strtoupper(substr(md5(microtime()),rand(0,26),7));
	
						if ($reward_detail['cJenis']=='Voucher Online'){//Jikaa jenis voucher adalah dapat digunakan untuk transaksi pemesanan makanan di saungsaji
							if($reward_detail['cVendor']=='SaungSaji'){
								if ($this->users_model->add_ms_voucher($reward_detail['iValue'],$code,'Saung',NULL)){
									if($this->users_model->add_tr_redeem_point($reward_detail['id'],1,$code)){
										$flag=1;										
										//send email ke user dan ke saungsaji
									}
								}
							}
							else{								
								...
								if ($resto){
									if ($this->users_model->add_ms_voucher($reward_detail['iValue'],$code,'Resto',$resto->id)){
										if($this->users_model->add_tr_redeem_point($reward_detail['id'],1,$code)){
											$flag=1;
											//send email ke user, ke saungsaji dan ke restoran
										}
									}
								}
							}
	...
	function email_activation($id,$type,$session){
		$this->load->model('users_model', '', TRUE);
		$res = $this->users_model->get_user_profile($id,$type,$session)->row();
		//echo $res->num_rows();		
        if($res){
			$this->db->where('id', $id);
            $this->db->update('msusers', array('cEmail'=>$res->cEmailNew,'dUpdated'=>date('Y-m-d H:i:s')));
            $data['msg']="Email pendaftaran Anda telah berhasil diganti menjadi <b>".$res->cEmailNew."</b>";
        }
        else $data['msg']="<font color='red'>Penggantian email gagal. Silahkan cek email Anda kembali.</font><br><br>";
        $data['base']=$this->base;
		$data['title']="SaungSaji.com - online food order and delivery";
		$this->load->view('header20150331', $data);		
		//$this->load->view('menu', $data);
		$this->load->view('email_activate', $data);
		$this->load->view('login', $data);
		$this->load->view('footer', $data);		
	}
	...
	function generate(){	
		...
		for ($i=0; $i<495; $i++){
			$code=strtoupper(substr(md5(microtime()),rand(0,26),7));
			if($this->users_model->add_ms_voucher(1000,$code,'Saung',NULL)){
				$a=$a+1;
			}
		}
		echo "Code to be generated = ".$a;
	}
